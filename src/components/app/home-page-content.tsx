...
export default function HomePageContent() {
  ...
  const [view, setView] = useState<ViewMode>(
    initialView === "recipient" ? "recipient" : "admin",
  );
  ...
  const isRecipientSession =
    initialView === "recipient" && !!projectIdFromUrl && !!recipientIdFromUrl;
  ...
  useEffect(() => {
    ...
    const fetchProject = async () => {
      ...
      try {
        ...
        if (
          recipientIdFromUrl &&
          fetchedProject.recipients.some((r) => r.id === recipientIdFromUrl)
        ) {
  ...
  const handleProjectChange = (newProject: Project) => {
    ...
    if (!isRecipientSession) {
      ...
      if (
        activeRecipientId &&
        !newProject.recipients.find((r) => r.id === activeRecipientId)
      ) {
          ...
          projectIdFromUrl && isLoadingProject ? (
            <div className="flex flex-col items-center justify-center text-center py-20">
              ...
              <p className="text-muted-foreground">
                Por favor, aguarde enquanto buscamos os dados do projeto.
              </p>
            </div>
          ) : (
